...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    ...
    while i < n:
        ...
        if tag.startswith("B-"):
            ...
            entity_tokens = [tokens[i]]
            i += 1
            while i < n and tags[i] == f"I-{bio_type}":
                entity_tokens.append(tokens[i])
                i += 1
            end = i - 1
            surface = "".join(entity_tokens)
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT NER 评估 — 新数据集格式")
...
def main():
    ...
    # ── 加载新格式数据 ──────────────────────────────────────────────────────────
    val_file = Path(args.data_dir) / "validation.json"
    if not val_file.exists():
        print(f"[错误] 验证数据文件不存在：{val_file}")
        return
    ...
    for sample in val_data[:5]:
        if "tokens" not in sample or "ner_tags" not in sample:
            print("[错误] 数据格式错误，缺少 'tokens' 或 'ner_tags' 字段")
            return
    ...
    for i, record in enumerate(samples, 1):
        ...
        detail_records.append({
            "text": text,
            "gold":  [{"text": s, "type": t} for s, t, _, _ in g_set],
            "pred":  [{"text": s, "type": t} for s, t, _, _ in p_set],
            "raw_output": raw,
        })
    ...
    if llm_log_path.exists():
        ...
        # 注意：llm_data 中可能是旧 10 类或新 3 类，这里仅用于对比
        llm_zero_f1 = f"{llm_data['zero_shot']['f1']:.4f}"
        llm_few_f1  = f"{llm_data['few_shot']['f1']:.4f}"

    print(f"\n{'='*65}")
    print(f"LLM SFT NER 评估结果（实体类型：{', '.join(ENTITY_TYPES)}）")
    print(f"{'='*65}")
