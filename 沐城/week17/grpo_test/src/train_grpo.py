...
if not hasattr(vllm_dist_utils, 'StatelessProcessGroup'):
    # 尝试从可能的模块导入
    possible_locations = [
        'vllm.distributed.parallel_state',
        'vllm.distributed.communication_op',
        'vllm.distributed.device_communicators.pynccl_utils',  # 某些版本可能在此
    ]
    found = False
    for loc in possible_locations:
        try:
            mod = __import__(loc, fromlist=['StatelessProcessGroup'])
            if hasattr(mod, 'StatelessProcessGroup'):
                setattr(vllm_dist_utils, 'StatelessProcessGroup', mod.StatelessProcessGroup)
                print(f"✅ StatelessProcessGroup 从 {loc} 导入成功")
                found = True
                break
        except ImportError:
            continue
    if not found:
        # 如果都找不到，创建一个占位符（可能导致后续错误）
        class DummyStatelessProcessGroup:
            pass
        setattr(vllm_dist_utils, 'StatelessProcessGroup', DummyStatelessProcessGroup)
        print("⚠️ 警告: StatelessProcessGroup 未找到，使用占位类")
...
MODEL_PATH = Path(r"/root/autodl-tmp/pretrain_models/Qwen2-0.5B-Instruct")
OUT_DIR = ROOT / "outputs"

# 训练集难度配比：依据 baseline_probe 的 loose_informative_group_rate 选择
# L3 (0.76) / L5 (0.66) 为主，L2 (0.68) 保底；L1/L4/L6 不进训练集，留作泛化评估
LEVEL_MIX = [
    ("L2_minus_2digit", 0.50),
    ("L1_minus_1digit", 0.25),
    ("L3_minus_3digit", 0.25),
]
